import {parseError} from "./misc.js";
...
class WikaNetwork {
  ...
  connect = (callback) => {
    ...
    return ApiPromise.create({provider: self.wsProvider}).then((api) => {
      self.api = api;
      callback();
    });
  };
  ...
  getUrlInfo = (address, url, callback) => {
    Promise.all([this.api.query.likes.urls(url), this.api.query.likes.likes(address, url)]).then((result) => {
      let ans = {
